...
class TimeseriesTest < Test::Unit::TestCase
  ...
  def assert_transform(input, transformer, expected_times)
    actual_times = []
    input.each do |itime|
      transformer.each_to(itime) do |otime|
        actual_times << otime.strftime("%Y-%m-%d %H:%M:%S %Z")
      end
      actual_times << "--"
    end
  ...
  def test_transformer_yields_each_step_in_series_until_input_time
    a = Timeseries.create(
      :start_time => Time.zone.parse("2010-01-01 00:15:00 UTC"),
      :stop_time  => Time.zone.parse("2010-01-01 00:45:00 UTC"),
      :period     => {:minutes => 15},
    )
    b = Timeseries.create(
      :start_time => Time.zone.parse("2010-01-01 00:00:00 UTC"),
      :stop_time  => Time.zone.parse("2010-01-01 00:35:00 UTC"),
      :period     => {:minutes => 5},
    )

    transformer = b.transformer
    assert_transform(a, transformer, [
      "2010-01-01 00:00:00 UTC",
      "2010-01-01 00:05:00 UTC",
      "2010-01-01 00:10:00 UTC",
      "2010-01-01 00:15:00 UTC",
      "--",
      "2010-01-01 00:20:00 UTC",
      "2010-01-01 00:25:00 UTC",
      "2010-01-01 00:30:00 UTC",
      "--",
      "2010-01-01 00:35:00 UTC",
      "--",
    ])
  end

  def test_transformer_synchronizes_with_input_series_if_start_time_is_nil
    a = Timeseries.create(
      :start_time => Time.zone.parse("2010-01-01 00:15:00 UTC"),
      :stop_time  => Time.zone.parse("2010-01-01 00:45:00 UTC"),
      :period     => {:minutes => 15},
    ...
    transformer = b.transformer(:start_time => nil)
    assert_transform(a, transformer, [
      "2010-01-01 00:15:00 UTC",
      "--",
      "2010-01-01 00:20:00 UTC",
  ...
  def test_transformer_continues_indefinitely_if_stop_time_is_nil
    a = Timeseries.create(
      :start_time => Time.zone.parse("2010-01-01 00:15:00 UTC"),
      :stop_time  => Time.zone.parse("2010-01-01 00:45:00 UTC"),
      :period     => {:minutes => 15},
    ...
    transformer = b.transformer(:stop_time => nil)
    assert_transform(a, transformer, [
      "2010-01-01 00:00:00 UTC",
      "2010-01-01 00:05:00 UTC",
      "2010-01-01 00:10:00 UTC",
      ...
      "--",
      "2010-01-01 00:35:00 UTC",
      "2010-01-01 00:40:00 UTC",
      "2010-01-01 00:45:00 UTC",
      "--",
    ])
  end
